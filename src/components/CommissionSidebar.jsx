...
function PolicyDrillDown({ policies, title, onBack }) {
  ...
  const thStyle = { padding: '4px 3px', fontSize: 8, color: C.accent, fontWeight: 600, cursor: 'pointer', userSelect: 'none', whiteSpace: 'nowrap' };
  const tdStyle = { padding: '3px', fontSize: 8, borderBottom: `1px solid ${C.border}33` };
...
export default function CommissionSidebar({ open, onClose }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [groupBy, setGroupBy] = useState('status'); // 'carrier' | 'status'
  ...
  const thStyle = { padding: '4px 3px', fontSize: 8, color: C.accent, fontWeight: 600, whiteSpace: 'nowrap' };
  const tdStyle = { padding: '3px', fontSize: 8 };
  ...
  const renderCarrierView = () => {
    ...
    return carrierOrder.map(([carrier, data]) => {
      ...
      return (
        <tbody key={carrier}>
          <tr><td colSpan="5" style={{ padding: '6px 3px 3px', color: C.text, fontWeight: 700, fontSize: 9, borderTop: `2px solid ${C.border}` }}>
  ...
  const renderStatusView = () => {
    ...
    return statusOrder.map(([status, data]) => {
      ...
      return (
        <tbody key={status}>
          <tr><td colSpan="5" style={{ padding: '6px 3px 3px', fontWeight: 700, fontSize: 9, borderTop: `2px solid ${C.border}` }}>
  ...
  return (
    <div style={{
      position: 'fixed', top: 0, right: 0, width: 420, height: '100vh', zIndex: 60,
      ...
      <div style={{ padding: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: C.accent, fontWeight: 700 }}>Commission Tracker</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ display: 'flex', gap: 1, background: C.card, borderRadius: 4, padding: 1, border: `1px solid ${C.border}` }}>
              <span onClick={() => { setGroupBy('carrier'); setDrillDown(null); }}
                style={{ padding: '3px 8px', borderRadius: 3, fontSize: 9, fontWeight: 600, cursor: 'pointer',
                  background: groupBy === 'carrier' ? C.accent : 'transparent', color: groupBy === 'carrier' ? '#fff' : C.muted }}>Carrier</span>
              <span onClick={() => { setGroupBy('status'); setDrillDown(null); }}
                style={{ padding: '3px 8px', borderRadius: 3, fontSize: 9, fontWeight: 600, cursor: 'pointer',
                  background: groupBy === 'status' ? C.accent : 'transparent', color: groupBy === 'status' ? '#fff' : C.muted }}>Status</span>
            </div>
            <span onClick={onClose} style={{ cursor: 'pointer', color: C.muted, fontSize: 14, lineHeight: 1 }}>✕</span>
          </div>
        </div>
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontSize: 8, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4, color: C.accent }}>Policies by Status</div>
                <DonutChart slices={pieSlices} centerTop={String(totals.count)} centerBottom="POLICIES" />
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 4, marginTop: 3, fontSize: 7.5 }}>
              ...
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontSize: 8, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4, color: C.accent }}>Commission $</div>
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
              {[
                ...
                <div key={w.label} style={{ flex: 1, background: C.card, border: `1px solid ${C.border}`, borderRadius: 5, padding: '4px 5px', textAlign: 'center' }}>
                  <div style={{ fontSize: 7, textTransform: 'uppercase', marginBottom: 1 }}>{w.label}</div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: w.good ? C.green : C.red }}>{w.val}</div>
                  {w.pct !== 0 && <div style={{ fontSize: 8, color: w.good ? C.green : C.red }}>{fmtPct(w.good ? Math.abs(w.pct) : -Math.abs(w.pct))}</div>}
                </div>
              ))}
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 7 }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                  <th style={{ ...thStyle, textAlign: 'left' }}>{groupBy === 'carrier' ? 'Status' : 'Carrier'}</th>
              ...
              {groupBy === 'carrier' ? renderCarrierView() : renderStatusView()}
              <tfoot>
                <tr style={{ borderTop: `2px solid ${C.accent}` }}>
                  <td style={{ ...tdStyle, fontWeight: 700, color: C.text, fontSize: 9, padding: '5px 3px' }}>TOTAL</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: C.text, padding: '4px 2px' }}>{totals.count}</td>
                  <td style={{ ...tdStyle, textAlign: 'center', fontWeight: 700, padding: '4px 2px' }}>
                    <span style={{ color: C.green }}>{totals.paid}</span>/<span style={{ color: C.red }}>{totals.unpaid}</span>
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: C.green, fontFamily: C.mono, padding: '4px 2px' }}>{fmtDollarFull(Math.round(totals.received))}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: C.yellow, fontFamily: C.mono, padding: '4px 2px' }}>{fmtDollarFull(Math.round(totals.balance))}</td>
                </tr>
              </tfoot>
            </table>
          </>
